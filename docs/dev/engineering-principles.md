...
- **Tradeoff:** Each project carries an explicit plugin pin and generated adapters, while the heavy marketplace payload is shared through the XDG cache. This adds dependency metadata to projects, but avoids vendoring HELIX/DDx asset trees into every repository and prevents "works on my machine" failures from stale global installs.
