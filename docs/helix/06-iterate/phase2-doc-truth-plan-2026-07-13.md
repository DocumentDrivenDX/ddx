...
**Date:** 2026-07-13
**Status:** Revised r3 — review remediation applied 2026-07-13; ready when the entry and work-breakdown dependencies below are met
...
Non-goals: writing new feature specs (Phase 1 owns FEAT-030 and the ADR-024 amendment); re-architecting code beyond deleting facades and dead code; prose style; website content. **Cross-phase edit ownership (review-expanded):** Phase 1 owns ADR-024, FEAT-030, and the CONTRACT-003/FEAT-006 typed-results amendment; FEAT-022/TD-033 review-model text and FEAT-006 execution-contract text are jointly owned (ledger rows 3–4 sequenced behind Phase 1 WB-3/WB-1); Phase 3's FEAT-002 "Known limitations" note lands inside this phase's FEAT-002 re-stamp commit.

### Entry criteria

Phase 0 WB-1 (freeze) in effect. WB-1, WB-2, WB-3, and WB-5 may proceed from that point according to their local dependencies. WB-4 may not be scheduled until Phase 0 WB-6 has made both `cd cli && env HOME=/nonexistent go test ./...` and `cd cli && go test ./...` green and established the scoped-gate/async-full-suite policy. Phase 1 does not block the whole phase: only contradiction-ledger rows 3 and 4 wait for Phase 1 WB-3 and WB-1 respectively.

### Exit criteria (revised — each mechanical)

1. Spec-honesty lint green in CI over `docs/helix/01-frame/features/` and all five subdirectories of `docs/helix/02-design/` (adr, concepts, contracts, solution-designs, technical-designs — review: the original scope skipped concepts/ and contracts/, where CONTRACT-003 lives). Every Complete/Implemented document has full requirement-to-verification coverage and a passing observation produced for the current revision; a waiver cannot satisfy this rule.
...
4. **Phantom-command sweep green, as a doc-lint rule:** every `ddx <cmd>` and shell command advertised by `ddx --help`, CLAUDE.md, and README exists (registered cobra command / real script). Known phantoms at review time: `ddx templates list/apply`, `ddx mcp list/install`, `ddx try --no-merge`, and `bun run houdini:generate` — the sweep must catch all four, and the rule stays in CI so new ones cannot land.
...
- Rows (corrections bolded):
  ...
  4. `discovered_subtasks` vs in-band create — FEAT-006 MUST NOT (`:376`) vs shipped prompt (`execute_bead.go:2029,2114,2150`). Decision: accept guard-railed in-band creation; **coordinated with Phase 1 WB-1's FEAT-006/CONTRACT-003 amendment — one FEAT-006 edit, jointly owned**; bead filed to extend the create-guard beyond fixture titles.
...
- Acceptance: every row Resolved or Sequenced-with-owner; duplicate-id lint passes; operator sign-off on the decision column before the batch applies.
- Validation: spechonesty lint; ledger review.
- Non-scope: contradictions not evidenced in the AR (new finds get new rows).
- Dependencies: WB-1; rows 3–4 gated on Phase 1 WB-3/WB-1.
...
- Files or systems and steps, by class:
  ...
  2. **comparisonDispatch + workerDispatch placeholder kinds — feature removal with UI and e2e surface** (review: the efficacy page calls comparisonDispatch at `+page.svelte:195` with a 388-line `efficacy.spec.ts`; the project-overview page renders "Re-align specs"/"Run checks" buttons at `+page.svelte:64,74` asserted by `actions.spec.ts:78`): remove the resolver paths (`resolver_feat008.go:183-227`), the action cards and comparison-dispatch UI, and update/remove both Playwright specs; ledger row 14 retires the FEAT-008 stories. Sized as its own bead(s), not folded into dead-code sweeps.
...
- Acceptance: exit criteria 3 and 4; deadcode check green without anchors; both Phase 0 WB-6 hermetic full-suite commands remain green; `bun run check` + `bun run test` green (Playwright specs updated, not deleted-without-replacement); CI byte-identical skill-tree check in place.
...
WB-1 and WB-4 are ordinary code beads (P7 rubric, corrected anchors). WB-1 may be filed once Phase 0 WB-1 is in effect; WB-4 beads must declare `IP-2026-07-13-phase0-stop-self-harm` WB-6 and this plan's WB-1 as dependencies, and the WB-4.2 bead must also wait for ledger row 14 approval. WB-2/WB-3/WB-5 are operator-reviewed doc batches applied directly, with the ledger as the record. WB-3 rows 3 and 4 remain explicitly sequenced behind `IP-2026-07-13-phase1-lower-altitude` WB-3 and WB-1; all other Phase 2 work remains parallel with Phase 1. The exit note appended here includes the current-revision verification report, ledger state, hermetic full-suite results, and phantom-command sweep output.
