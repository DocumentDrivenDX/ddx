...
registration ticks, blueprint-grid bleed, and a small `DDx · FIG.P3`
...
**`DDx · FIG.P3`** in JetBrains Mono 0.625 rem `#6B7280`, and at top-left a
...
diagram itself (one shared lever, plural interchangeable methodology grips) must remain the dominant read at first glance —
the industrial framing rewards the second look without competing with the
metaphor.

# Anchoring (recently-landed positioning)

Anchored to **Methodology is plural** and to **Avoid Vendor Lock-in** (`/docs/principles/avoid-vendor-lock-in/`). DDx is the platform; HELIX, agile, waterfall, and ad-hoc are interchangeable plugins that snap into the same primitives — beads, runs, artifacts. The factory line accepts any methodology jig.
