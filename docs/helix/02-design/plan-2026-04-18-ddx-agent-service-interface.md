...
| `cli/internal/agent/jsonl.go`, `format.go`, `session_log_format.go`, `session_log_tailer.go` | **Mostly move** to ddx-agent (event/format primitives); DDx keeps only copied/linkable evidence-envelope metadata and attachment pointers, and it does not render or parse the inner Fizeau transcript or session-log content. Fizeau owns the session-log rendering surface and transcript semantics |
