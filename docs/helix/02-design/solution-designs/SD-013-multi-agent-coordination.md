...
**Authentication:** Non-localhost access requires an API key configured in
`ddx-server.yml`. The server rejects write requests without a valid key when
`auth.required: true` is set. Read endpoints may optionally require auth as
well. Auth configuration is out of scope for this design; the interface point
is defined here for FEAT-002 to implement.
