...
- **`POST /graphql`** powered by `gqlgen` — schema-first, codegen produces typed
  Go resolvers.
- **`schema.graphql`** is the single source of truth. TypeScript client generated
  from it via Houdini.
...
- **Houdini** as the GraphQL client — SvelteKit-sanctioned, codegen from
  `schema.graphql`, typed `load()` functions and typed subscriptions.
...
Houdini codegen produces typed responses.
...
data pulled via Houdini from the Stage 2 GraphQL endpoint. User runs the binary
and confirms.
