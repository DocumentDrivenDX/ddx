...
| `decompose_applied` — decomposer filed 2-5 executable child, sibling, or replacement beads losslessly; oversized bead is closed as completed-by-decomposition | `in_progress → closed` (`completed-by-decomposition`) on lossless split; `in_progress → proposed` only when decomposition is lossy or requires operator judgment | add `decomposed` label | `decompose-applied` with generated bead IDs + `reframer_cost` + `closed-completed-by-decomposition` | `extra.children` or `extra.superseded-by` + AC mapping; `extra.last-recovery` updated |
