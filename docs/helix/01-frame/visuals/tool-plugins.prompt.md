...
registration ticks, blueprint-grid bleed, and a small `DDx · FIG.T3`
...
**`DDx · FIG.T3`** in JetBrains Mono 0.625 rem `#6B7280`, and at top-left a
...
diagram itself (plugins snapping into the shared DDx core) must remain the dominant read at first glance —
the industrial framing rewards the second look without competing with the
metaphor.

# Anchoring (recently-landed positioning)

Anchored to **Avoid Vendor Lock-in** and to the platform/workflow split documented in vision (`Platform and Workflow`). DDx ships primitives — library, beads, agents, personas — through named sockets. HELIX, Checks, and your own packs snap in. Workflow opinions live in plugins, never the core.
