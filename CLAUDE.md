...
- `library/` - DDx default plugin source (installed to `ddxroot.Path()/plugins/ddx/` by `ddx init`, which resolves to `<projectRoot>/.ddx/plugins/ddx/` in-tree or `${XDG_DATA_HOME}/ddx/projects/<identity>/plugins/ddx/` in convention mode). `ddx install <name>` is for DDx project package resources. Agent-facing skills are distributed from `DocumentDrivenDX/ddx-library` through the plugin marketplace path, preferably `npx claude-plugins install @DocumentDrivenDX/ddx-library/ddx`; do not recommend `ddx install ddx --force` as the skills install path.
...
Plugin and persona lookup follows project > global > baked-in precedence (`ddx-937de9fc`): project-local (`ddxroot.Path()/plugins/<name>/`, which is `<project>/.ddx/plugins/<name>/` in-tree or `${XDG_DATA_HOME}/ddx/projects/<identity>/plugins/<name>/` in convention mode) → global install (`${XDG_DATA_HOME}/ddx/global/plugins/<name>/`) → baked-in binary default (only for the `ddx` plugin). The project layer always wins when present. `ddx doctor` reports both the global install layer and the project install layer, including when the project copy is absent and falls through to the global layer (`lazy-resolves-to-global`). Universal agent-facing skill installation is handled by the marketplace package in `DocumentDrivenDX/ddx-library`, not by `ddx install ddx --force`.
