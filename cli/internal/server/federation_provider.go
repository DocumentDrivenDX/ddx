...
//
// Owner-targeted writes go through ForwardMutation: the hub resolves the
// project owner, refuses offline/stale/read-only spokes without a hub-local
// write fallback, stamps origin/request-id headers, and replays cached
// responses for a repeated RequestID / IdempotencyKey.
...
// the spoke can enforce origin and idempotency constraints. Offline, stale, and
// read-only spokes are refused without any hub-local write fallback. A repeated
// RequestID / IdempotencyKey returns the cached response without a second POST.
func (p *hubFederationProvider) ForwardMutation(ctx context.Context, req *federation.ForwardMutationRequest) (*federation.ForwardMutationResponse, error) {
	...
	resp, err := p.fanOutClient().ForwardMutation(ctx, &targetCopy, req.Body, headers)
	if err != nil {
		return nil, err
	}
...
func spokeAllowsForwardMutation(caps []string) bool {
	...
	// Legacy spokes with no non-identity capabilities still accept writes.
	return advertised == 0
}
