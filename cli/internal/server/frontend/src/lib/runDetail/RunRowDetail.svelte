<script lang="ts">
	...
	let { runId, layer, nodeId, projectId }: Props = $props()
	...
	async function fetchSession(sessionId: string) {
		...
		try {
			const client = createClient()
			const data = await client.request<{ agentSession: SessionDetail | null }>(
				RUN_SESSION_QUERY,
				{ id: sessionId }
			)
	...
	<div data-active-tab={activeTab}>
		...
		{:else if activeTab === 'prompt'}
			<Prompt
				...
				path={layer === 'try' ? exec?.promptPath ?? (exec ? `${exec.bundlePath}/prompt.md` : null) : null}
			/>
		{:else if activeTab === 'response'}
			<Response
				...
				path={layer === 'try' ? exec?.resultPath ?? (exec ? `${exec.bundlePath}/result.json` : null) : null}
			/>
