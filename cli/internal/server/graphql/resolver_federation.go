...
type FederationProvider interface {
	...
	// metadata needed to preserve the write contract. Implementations must
	// refuse offline/stale/read-only spokes and may replay cached responses
	// for a repeated RequestID / IdempotencyKey without a second spoke POST.
