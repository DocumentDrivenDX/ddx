...
func ExistingPath(ctx context.Context, projectRoot string) (string, bool) {
	...
	if info, err := os.Stat(filepath.Join(inTree, "config.yaml")); err == nil && !info.IsDir() {
		return inTree, true
	}
...
//
// When the project already has an initialized in-tree `.ddx/config.yaml`,
// that remains authoritative. Otherwise DDx falls back to an XDG-scoped
// convention root derived from the project's canonical git remote, or a
