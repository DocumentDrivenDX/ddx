...
// TestBeadDoctorDetectsAndRepairsBackEdge covers the new parent-ancestor dep
// scan: a bead with dependency edges to both its direct parent and a parent
// ancestor must be reported deterministically and repaired by pruning only the
// offending edges.
func TestBeadDoctorDetectsAndRepairsBackEdge(t *testing.T) {
	...
	for _, finding := range report.Findings {
		assert.Equal(t, DoctorFindingCodeParentAncestorInDeps, finding.Code)
	...
	require.NotNil(t, childRow, "repaired tracker must still contain the child row")
	deps, ok := childRow["dependencies"].([]any)
	require.True(t, ok, "repair must preserve non-offending dependency edges")
	require.Len(t, deps, 1, "only the healthy dependency should survive the fix")
	dep := deps[0].(map[string]any)
	assert.Equal(t, "ddx-helper", dep["depends_on_id"])

	reportAfter, err := BeadDoctor(path)
	require.NoError(t, err)
	assert.True(t, reportAfter.Clean(), "after pruning parent-ancestor deps the tracker must be clean")
}
