...
func TestPolicy_UnknownModeFallsBackToOperatorRequired(t *testing.T) {
	p := DefaultPolicy()
	got := p.Decide("ddx-test", FailureMode("not_a_mode"), nil)
	if got != ActionOperatorRequired {
		t.Fatalf("unknown mode: got %q want %q", got, ActionOperatorRequired)
	}
}
...
func TestPolicy_ConfigOverride(t *testing.T) {
	...
	if got != ActionOperatorRequired {
		t.Fatalf("override second rung: got %q want %q", got, ActionOperatorRequired)
	}
	// A mode absent from the override map yields ActionOperatorRequired.
	got = custom.Decide("ddx-test", FailureModeNoChanges, nil)
	if got != ActionOperatorRequired {
		t.Fatalf("absent mode: got %q want %q", got, ActionOperatorRequired)
	}
}
...
func TestParseAction(t *testing.T) {
	...
	if _, err := ParseAction("operator_required"); err != nil {
		t.Fatalf("operator_required action rejected: %v", err)
	}
	if _, err := ParseAction("needs_human"); err == nil {
		t.Fatalf("legacy needs_human action accepted")
	}
