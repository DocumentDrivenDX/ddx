...
func TestCloseAbandonedForPIDAppendsRecoveredRelease(t *testing.T) {
	root := t.TempDir()
	path := Path(root)
	require.NoError(t, os.MkdirAll(filepath.Dir(path), 0o755))
	acquiredAt := time.Date(2026, 6, 17, 12, 36, 22, 0, time.UTC)
	releasedAt := acquiredAt.Add(3 * time.Second)
	pid := 424242

	require.NoError(t, appendEvent(path, Event{
		Event:      "acquire",
		LockName:   "index.lock",
	...
	closed, err := CloseAbandonedForPID(root, pid, releasedAt, "worker exited")
	require.NoError(t, err)
	...
	assert.Equal(t, pid, release.HolderPID)
	assert.Equal(t, acquiredAt.Format(time.RFC3339Nano), release.AcquiredAt)
	assert.Equal(t, releasedAt.Format(time.RFC3339Nano), release.ReleasedAt)
	assert.Equal(t, int64(3000), release.DurationMS)
	assert.Equal(t, "error", release.Severity)
	assert.True(t, release.Recovered)
	assert.Equal(t, "worker exited", release.Reason)
}

func TestCloseAbandonedForPIDIgnoresMatchedAcquire(t *testing.T) {
	root := t.TempDir()
	path := Path(root)
	require.NoError(t, os.MkdirAll(filepath.Dir(path), 0o755))
	acquiredAt := time.Date(2026, 6, 17, 12, 36, 22, 0, time.UTC)
	releasedAt := acquiredAt.Add(50 * time.Millisecond)
	pid := 424243
	...
	for _, ev := range []Event{
		...
		require.NoError(t, appendEvent(path, ev))
	}

	closed, err := CloseAbandonedForPID(root, pid, releasedAt.Add(time.Second), "worker exited")
	require.NoError(t, err)
