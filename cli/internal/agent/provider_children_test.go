...
// TestWork_NonRouteProviderPidNotProcessGroupLeaderIsReaped lives in
// provider_children_linux_test.go: it asserts on /proc-derived zombie/dead
// process state via processDeadOrZombieStatus, which is Linux-only (see
// process_dead_or_zombie_linux_test.go).
