...
func TestExecuteBeadWorker_ClaimLeaseStaysFreshDuringPreClaimIntake(t *testing.T) {
	...
	bead.HeartbeatInterval = 10 * time.Millisecond
	bead.HeartbeatTTL = 40 * time.Millisecond
	t.Cleanup(func() {
	...
	store := bead.NewStore(t.TempDir())
	require.NoError(t, store.Init(context.Background()))
	only := &bead.Bead{ID: "ddx-intake-heartbeat", Title: "intake heartbeat", Priority: 0}
	require.NoError(t, store.Create(context.Background(), only))

	intakeStarted := make(chan struct{})
	...
	for time.Now().Before(deadline) {
		fresh, found, err := store.ClaimHeartbeatFresh(only.ID)
		require.NoError(t, err)
		require.True(t, found, "claim heartbeat sidecar must exist while intake is blocked")
		require.True(t, fresh, "claim heartbeat must stay fresh while intake is blocked")
		time.Sleep(bead.HeartbeatInterval)
	}
...
func TestExecuteBeadWorker_StaggeredSecondWorkerCannotReclaimDuringIntake(t *testing.T) {
	...
	store := bead.NewStore(t.TempDir())
	require.NoError(t, store.Init(context.Background()))
	only := &bead.Bead{ID: "ddx-intake-no-reclaim", Title: "intake no reclaim", Priority: 0}
	require.NoError(t, store.Create(context.Background(), only))
	...
	makeWorker := func() *ExecuteBeadWorker {
	...
	makeConfig := func(assignee string) config.ResolvedConfig {
		opts := config.TestLoopConfigOpts{
			Assignee:          assignee,
			HeartbeatInterval: bead.HeartbeatInterval,
		}
	...
	go func() {
		_, err := makeWorker().Run(context.Background(), makeConfig("worker-a"), ExecuteBeadLoopRuntime{
			Once: true,
			PreClaimIntakeHook: func(ctx context.Context, beadID string) (PreClaimIntakeResult, error) {
				close(intakeStarted)
				select {
	...
	case <-intakeStarted:
	case <-time.After(time.Second):
		t.Fatal("worker A intake hook never started")
	}
	time.Sleep(2 * bead.HeartbeatTTL)

	ready, err := store.ReadyExecution()
	require.NoError(t, err)
	for _, b := range ready {
		require.NotEqual(t, only.ID, b.ID, "worker B must not see worker A's live intake claim as ready")
	}

	resultB, err := makeWorker().Run(context.Background(), makeConfig("worker-b"), ExecuteBeadLoopRuntime{Once: true})
	require.NoError(t, err)
	require.NotNil(t, resultB)
	require.Equal(t, 0, resultB.Attempts, "staggered worker B must not reclaim the bead while worker A is live in intake")

	close(releaseIntake)
	require.NoError(t, <-doneA)
	require.Equal(t, int32(1), attempts.Load(), "only worker A's attempt should run")
}
