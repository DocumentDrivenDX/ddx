...
// TestFinalizeDurableAuditOrStop_IndexCommitLockViolationReleasesAndContinues
// proves that a cap violation during a durable-audit commit is recorded as a
// durable violation event, the stale index.lock is recovered, and a subsequent
// tracker-only or durable-audit commit proceeds without manual lock removal.
func TestFinalizeDurableAuditOrStop_IndexCommitLockViolationReleasesAndContinues(t *testing.T) {
