...
// newPluginInstallCommand creates the "ddx plugin install <name>" subcommand.
...
Examples:
  ddx plugin install helix                        # Install HELIX workflow (project-local)
  ddx plugin install helix --force                # Reinstall even if already up to date
  ddx plugin install helix --local ../helix --force
  ddx plugin install persona/strict-code-reviewer # Install a single persona`,
		Args: cobra.ExactArgs(1),
		RunE: f.runPluginInstall,
	}
...
func (f *CommandFactory) runPluginInstall(cmd *cobra.Command, args []string) error {
	out := cmd.OutOrStdout()
...
// ddx plugin install --local must not fail validation on broken symlinks that live
