...
func TestInitGitignoreRules(t *testing.T) {
	...
	assert.Contains(t, content, ".ddx/coordination/", ".ddx/coordination/ must be ignored")
	...
	// Verify with git check-ignore that concrete evidence files ARE ignored.
	...
	// git check-ignore exits 0 if ignored, 1 if not — every evidence file MUST be ignored.
	for _, evidence := range []string{
		...
		".ddx/executions/abc123/embedded/session.jsonl",
	} {
		ci := exec.Command("git", "check-ignore", "-q", evidence)
		ci.Dir = te.Dir
		assert.NoError(t, ci.Run(), evidence+" must be ignored by git")
	}
