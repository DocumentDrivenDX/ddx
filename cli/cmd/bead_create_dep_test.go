...
func TestBeadDepAddRejectsParentAncestor(t *testing.T) {
	...
	childOut, err := executeCommand(rootCmd, "bead", "create", "Child", "--type", "task", "--parent", parentID)
	require.NoError(t, err)
	childID := strings.TrimSpace(childOut)
	require.NotEmpty(t, childID)

	t.Run("direct-parent", func(t *testing.T) {
		_, err := executeCommand(rootCmd, "bead", "dep", "add", childID, parentID)
		require.Error(t, err)
		assert.Contains(t, err.Error(), "ancestor in the parent chain")
		assert.Contains(t, err.Error(), parentID)
		assert.Contains(t, err.Error(), parentID+" -> "+rootID)
	...
	t.Run("grandparent-ancestor", func(t *testing.T) {
		_, err := executeCommand(rootCmd, "bead", "dep", "add", childID, rootID)
		require.Error(t, err)
		assert.Contains(t, err.Error(), "ancestor in the parent chain")
		assert.Contains(t, err.Error(), rootID)
		assert.Contains(t, err.Error(), parentID+" -> "+rootID)
