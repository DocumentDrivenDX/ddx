...
func checkGitRepoHealth(workingDir string, fix bool) []DiagnosticIssue {
	issues := gitpkg.CheckGitRepoHealth(workingDir, fix)
	if len(issues) == 0 {
		return nil
	}
	converted := make([]DiagnosticIssue, 0, len(issues))
	for _, issue := range issues {
		converted = append(converted, DiagnosticIssue{
			Type:        issue.Type,
			Description: issue.Description,
			Remediation: append([]string(nil), issue.Remediation...),
		})
	}
	return converted
}
