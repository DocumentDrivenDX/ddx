...
Examples:
  ...
  ddx doc dependents helix.prd  # Show what depends on a document`,
	...
	cmd.AddCommand(f.newDocListCommand())
...
func (f *CommandFactory) newDocListCommand() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "list",
		Short: "List documents, optionally filtered by type prefix",
		Args:  cobra.NoArgs,
		RunE: func(cmd *cobra.Command, args []string) error {
			graph, err := f.buildDocGraph()
			if err != nil {
			...
			for _, id := range ids {
				if typeFilter != "" {
					prefix := strings.ToUpper(typeFilter)
					seg := docIDTypePrefix(id)
					if seg != prefix {
						continue
					}
				}
	...
	cmd.Flags().String("type", "", "Filter by document ID type prefix (e.g. ADR, SD, FEAT)")
	cmd.Flags().Bool("json", false, "Output as JSON")
	return cmd
}

// docIDTypePrefix extracts the leading type segment from a document ID.
// For "ADR-001" it returns "ADR"; for "helix.prd" it returns "".
func docIDTypePrefix(id string) string {
...
// newAdrLegacyCommand registers 'ddx adr' as a legacy redirect per the
// FEAT-005 migration table: 'ddx adr list' → 'ddx doc list --type ADR'.
func (f *CommandFactory) newAdrLegacyCommand() *cobra.Command {
	adrCmd := &cobra.Command{
		Use:   "adr",
		Short: "Legacy ADR commands (use 'ddx doc' with --type ADR instead)",
		Run: func(cmd *cobra.Command, args []string) {
			_ = cmd.Help()
		},
	}
	...
	listCmd := &cobra.Command{
		Use:   "list",
		Short: "List ADR documents (legacy: use 'ddx doc list --type ADR' instead)",
		Args:  cobra.NoArgs,
		RunE: func(cmd *cobra.Command, args []string) error {
			fmt.Fprintln(cmd.ErrOrStderr(), "warning: 'ddx adr list' is deprecated; use 'ddx doc list --type ADR'")
			graph, err := f.buildDocGraph()
			if err != nil {
				return err
			}
			...
			if len(entries) == 0 {
				fmt.Fprintln(cmd.OutOrStdout(), "No ADR documents found.")
				return nil
			}
			for _, e := range entries {
				fmt.Fprintf(cmd.OutOrStdout(), "%s  %s\n", e.ID, e.Path)
